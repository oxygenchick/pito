...
import {icon,shelf,poop} from './visuals.mjs';
...
test('start uses the original Cyrillic logo and live, exact tagline',()=>{
 ...
 assert.match(html,/data-act="new"/);
 assert.ok(existsSync(new URL('./assets/ui-v8/logo.png',import.meta.url)));
});
...
test('all illustrated UI objects use the approved raster sprite families',()=>{
 for(const id of ['apple','jar','cloud','care-basket','toy-box']){
  ...
  assert.ok(existsSync(new URL(`./assets/${version}/${id}.png`,import.meta.url)));
 }
});
test('font is local, Cyrillic-ready in both actual weights, served by dev server',()=>{
 ...
 for(const weight of ['Regular','Bold'])assert.ok(existsSync(new URL(`./assets/fonts/BalsamiqSans-${weight}.ttf`,import.meta.url)));
 assert.match(readFileSync(new URL('./index.html',import.meta.url),'utf8'),/href="fonts.css"/);
 assert.match(readFileSync(new URL('./server.mjs',import.meta.url),'utf8'),/font\/ttf/);
});
