...
import {pet,poop,icon,shelf} from './visuals.mjs';
import {gazeOffset,installGaze} from './character-gaze.mjs';
test('decorative icons are raster images, including former SVG symbols',()=>{
 for(const id of ['coin','heart','drop','rain','settings','task','ball','comic','puzzle','kite','console','bike','house','hand','arrow','lock','home','sun','check','close','star']){
  ...
  assert.ok(existsSync(new URL('./'+html.match(/src="([^"]+)"/)[1],import.meta.url)));
 }
});
...
test('pupil masks use exactly the visible whites for each open-eye emotion',()=>{
 const css=readFileSync(new URL('./character-v8.css',import.meta.url),'utf8');
 ...
 const current=readFileSync(new URL('./polish-v11.css',import.meta.url),'utf8');
...
test('eye sprites share registration and the pupil is a square circular sprite',()=>{
 const pngSize=name=>{const png=readFileSync(new URL(`./assets/${name==='eyes-happy'?'ui-v11':'pet-v8'}/${name}.png`,import.meta.url));return [png.readUInt32BE(16),png.readUInt32BE(20)];};
