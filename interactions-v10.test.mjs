...
test('high contrast object sprites are actual new images without badge markup',()=>{
 for(const name of ['cloud','hand','arrow']){const html=icon(name),path=name==='arrow'?'ui-v11/chevron.png':'ui-v10/'+name+'.png';assert.ok(html.includes(path));assert.ok(existsSync(new URL('./assets/'+path,import.meta.url)));}
...
test('grown event uses game-relative proportions and needs support exit animation',()=>{
 const css=readFileSync(new URL('./polish-v10.css',import.meta.url),'utf8');
