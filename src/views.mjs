...
export function modalView(s, ui) {
 ...
 switch (ui.modal) {
  ...
  case 'settings':
   title = 'Настройки';
   body = `<div class="settings-list"><button class="btn secondary wide" data-act="audio-toggle" aria-pressed="${ui.audioEnabled!==false}">Музыка и звуки: ${ui.audioEnabled===false?'выкл':'вкл'}</button>${button('Как играть','help','','secondary wide')}${button('Для взрослого','adult-gate','','secondary wide')}</div><p class="muted">Прогресс сохраняется автоматически.</p>`; break;
  case 'help':
   title = 'Как играть';
