...
import {ITEM_ART} from '../src/item-catalog.mjs';
import {shelf,icon} from '../src/visuals.mjs';
import {renderStart} from '../src/start-screens.mjs';
const catalog=[...E.ITEMS,...E.GOALS];
const read=p=>readFileSync(projectURL(p,import.meta.url),'utf8');
...
test('all eleven objects have individual real PNGs used by the renderer',()=>{
 for(const item of catalog){
  ...
  assert(existsSync(projectURL(path,import.meta.url)));
  const png=readFileSync(projectURL(path,import.meta.url));
