...
import {choosePlayPlan} from '../src/play-plan.mjs';
...
const appSource = readFileSync(projectURL('./app.mjs', import.meta.url), 'utf8');
...
test('open needs provide the hiding contract for the goal strip next to a grown pet', () => {
 ...
 if (/class="goal-strip"/.test(markup)) {
  const stylesheet = readFileSync(projectURL('./ux.css', import.meta.url),'utf8');
...
test('eleven collection objects remain visible on one rack and the ground without paging', () => {
 ...
 const css = readFileSync(projectURL('./shelf.css',import.meta.url),'utf8');
