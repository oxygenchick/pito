...
import {pet,icon} from './visuals.mjs';
import {renderStart} from './start-screens.mjs';
const css=readFileSync(new URL('./polish-v11.css',import.meta.url),'utf8');
...
test('decorative controls are image chevrons, plus and minus with preserved proportions',()=>{
 for(const id of ['chevron','plus','minus']){assert.match(icon(id),new RegExp('ui-v11/'+id+'.png'));assert(existsSync(new URL('./assets/ui-v11/'+id+'.png',import.meta.url)));}
